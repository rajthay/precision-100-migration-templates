...
set pages 0
set trimspool on
spool $MIG_PATH/output/reports/kw/cif/s_retail_phone.dat
select
'LEG_CUST_BRANCH'||'|'||
'LEG_CUST_NUMBER'||'|'||
'FIN_SOL_ID'||'|'||
...
inner join (select * from gfpf where gfpf.gfctp in('EC','EV','EW'))gfpf on trim(map_cif.gfclc)=trim(gfpf.gfclc) and  trim(map_cif.gfcus)=trim(gfpf.gfcus)
