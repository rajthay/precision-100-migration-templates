...
case
when R7FNR2||R7FNR3 is not null then RPAD(substr(R7FNR2||R7FNR3,1,50),50,' ')
else RPAD(' ',50,' ') end,
--CLOSURE_REMARKS CHAR(50 BYTE)
--RPAD(' ',50,' '),
rpad('BOF'||r0ab||r0an||r0as||r0bot||R0FRC||R0FRAB||R0FRAN||R0FRAS,50,' '),--this for report purpose changed on 23-06-2016
...
--case when r7bot = 'S' and r0min > 0 and r0max > 0 and r0min=r0max then 'F' else 'V' end, changed on 10-08-2016 as per Tanfeet request.
...
case
 when r0an='316050' then RPAD('L',1,' ')
 when r7bot = 'S' then 'L'
else RPAD(' ',1,' ') end,
...
case when trim(FIN_CR_CURRENCY)=trim(FIN_DR_CURRENCY) then lpad(' ',5,' ') 
when trim(FIN_CR_CURRENCY)<> trim(FIN_DR_CURRENCY) then lpad('CUS',5,' ') 
else lpad(' ',5,' ') end,
--Cr_Rate_code                 NVARCHAR2(5) 
case when trim(FIN_DR_CURRENCY)=trim(FIN_CR_CURRENCY) then lpad(' ',5,' ') 
when trim(FIN_DR_CURRENCY)<> trim(FIN_CR_CURRENCY) then lpad('CUB',5,' ') 
else lpad(' ',5,' ') end    
from BALANCE_ORDER
...
and r0an not in ('855100','855000')
and scact not in ('YL','YP')
and ((R0BOT in ('D','L','S') and R0frc ='F')or (R0BOT in ('S') and R0frc ='R'));
--and trim(r0frc) ='F'
commit;
update bo_cccy_o_table set sol_id= rpad(substr(trim(ACCOUNT_NUMBER),1,3),8,' ') where trim(SOL_ID) is null;
commit;
exit; 
