...
INSERT INTO SIU_CCCY_O_TABLE  
  select 
--SOL_ID    CHAR(8 BYTE)
rpad( nvl(fund_sol_id,' ') ,8,' '),
...
then RPAD('Y',1,' ') else RPAD('N',1,' ') end,--changed on 13-12-2015
...
case when trim(c8ccy)=trim(FUND_CCY) then lpad(' ',5,' ') 
when trim(c8ccy)<> trim(FUND_CCY) then lpad('SEL',5,' ') 
else lpad(' ',5,' ') end,
--Cr_Rate_code                 NVARCHAR2(5) 
case when trim(c8ccy)=trim(RECV_CCY) then lpad(' ',5,' ') 
when trim(c8ccy)<> trim(RECV_CCY) then lpad('BUY',5,' ') 
else lpad(' ',5,' ') end
...
COMMIT;
update SIU_CCCY_O_TABLE set sol_id= rpad(substr(trim(ACCOUNT_NUMBER),1,3),8,' ') where trim(SOL_ID) is null; 
commit;
exit; 
