...
insert into COL_TRADEABLE_O_TABLE
select distinct
-- COL_TYPE                 NVARCHAR2(1),
'S',
--  COL_CODE                 NVARCHAR2(8),
case when FIN_COL_CODE is null and hyclp='003' then 'KST-KWD'     
when FIN_COL_CODE is null and hyclp='004' then 'NST-KWD'
else FIN_COL_CODE 
end,
...
where hyclp in('003','004') and substr(HYCLR,10,3) not in('ABG','ABK') and  SUBSTR(HYCLR,32,4) not in('QUOT','UNQT','CASH')
...
;
commit;
update COL_TRADEABLE_O_TABLE set SERIAL_NUMBER='S'||rownum;
UPDATE COL_TRADEABLE_O_TABLE SET REVIEW_DATE=GET_PARAM('EOD_DATE') WHERE TO_DATE(REVIEW_DATE,'DD-MM-YYYY')<TO_DATE(GET_PARAM('EOD_DATE'),'DD-MM-YYYY') or TRIM(REVIEW_DATE) is null;
commit;
exit;